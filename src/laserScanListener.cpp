...
bool LaserScanListener::convertScanToPointCloud(sensor_msgs::LaserScan& scan)
{
	  ...
	  try
	  {
		  ...
		  ROS_INFO_STREAM("Before calling transformlaserscantopointcloud");
		  lprojector_.transformLaserScanToPointCloud(base_link_, scan, cloud, buffer_);
		  ROS_INFO_STREAM("After calling transformlaserscantopointcloud");
	  }
...
void LaserScanListener::detectBreakPoint(const sensor_msgs::LaserScan& scan)
{
	...
	ROS_INFO("LaserScanListener::detectBreakPoint: length of pcl cloud: %d", (int)pcl_cloud_->size());
    ...
    for(int i=1;i<scan.ranges.size();i++){
      	...
      	if(prev_range_set)
      	{
        	...
        	if(euc_dist > dmax)
        	{
	    		...
	    		ROS_INFO_STREAM( "LaserScanListener::detectBreakPoint: Starting from : " << s << " to :" << e);
...
void LaserScanListener::detectLineSegments(pcl::PointCloud<pcl::PointXYZ>::iterator begin, pcl::PointCloud<pcl::PointXYZ>::iterator end)
{
    ...
    for(ppit_ls = begin+1;ppit_ls<=end-1;ppit_ls++)
    {
		...
		//(ppit_ls+kf)->x,(ppit_ls+kf)->y,(ppit_ls-kb)->x,(ppit_ls-kb)->y,ppit_ls->x,ppit_ls->y);
      	...
      	ROS_INFO("LaserScanListener::detectLineSegments[Step 2]: Forward vector:[%f,%f], Backward Vector:[%f,%f]",f_vec[0],f_vec[1],b_vec[0],b_vec[1]);
      	...
      	ROS_INFO("LaserScanListener::detectLineSegments[Step 2]: Dot product: <%f>, Mag product: <%f>", dot_product,mag_product);
